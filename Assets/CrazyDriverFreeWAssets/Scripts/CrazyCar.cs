...
namespace EndlessCarChase
{
    ...
    public class CrazyCar : MonoBehaviour
    {
        ...
        [Tooltip("Make AI cars try to avoid obstacles. Obstacle are objects that have the CrazyObstacle component attached to them")]
        ...
        void Update()
        {
            ...
            if ( gameController.playerObject != this )
            {
                ...
                if ( avoidObstacles == true && Physics.Raycast(rayRight, out hit, detectDistance) && (hit.transform.GetComponent<CrazyObstacle>() || (hit.transform.GetComponent<CrazyCar>() && gameController.playerObject != this)) )
                {
                ...
                else if ( avoidObstacles == true && Physics.Raycast(rayLeft, out hit, detectDistance) && (hit.transform.GetComponent<CrazyObstacle>() || (hit.transform.GetComponent<CrazyCar>() && gameController.playerObject != this))) // Otherwise, if we detect an obstacle on our left side, swerve to the right
                {
                    // Change the emission color of the obstacle to indicate that the car detected it
                    //if (hit.transform.GetComponent<MeshRenderer>()) hit.transform.GetComponent<MeshRenderer>().material.SetColor("_EmissionColor", Color.red);
        ...
        void OnTriggerStay(Collider other)
        {
            ...
            if ( hurtDelayCount <= 0  && other.GetComponent<CrazyCar>() )
            {
                // Reset the hurt delay
                hurtDelayCount = hurtDelay;

                // Damage the car
                other.GetComponent<CrazyCar>().ChangeHealth(-damage);
