...
namespace EndlessCarChase
{
    ...
    /// </summary>
    public class CrazyObstacle : MonoBehaviour
    {
        ...
        void OnTriggerStay(Collider other)
        {
            ...
            if (other.GetComponent<CrazyCar>() )
            {
                //if ( other.GetComponent<CrazyCar>().hurtDelayCount <= 0 )
                //{
                    // Reset the hurt delay
                    //other.GetComponent<CrazyCar>().hurtDelayCount = other.GetComponent<CrazyCar>().hurtDelay;

                    // Damage the car
                    other.GetComponent<CrazyCar>().ChangeHealth(-damage);

                    // If there is a hit effect, create it
                    if (other.GetComponent<CrazyCar>().health - damage > 0 && other.GetComponent<CrazyCar>().hitEffect) Instantiate(other.GetComponent<CrazyCar>().hitEffect, transform.position, transform.rotation);
                //}
